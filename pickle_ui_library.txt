...
local function CleanupAllBlur()
    for _, effect in pairs(Lighting:GetChildren()) do
        if effect:IsA("BlurEffect") and effect.Name:find("Pickle") then
            effect:Destroy()
        end
    end
end
...
function PickleLibrary:CreateWindow(options)
    options = options or {}
    
    CleanupAllBlur()
    if GuiParent:FindFirstChild("PickleUI") then
        GuiParent:FindFirstChild("PickleUI"):Destroy()
    end
    
    local windowName = options.Name or "Pickle"
    local windowSubtitle = options.Subtitle or "UI Library"
    local windowVersion = options.Version or "v2.0"
    local loadingTitle = options.LoadingTitle or "Pickle Interface"
    ...
    local configFolder = configSaving.FolderName or "PickleConfig"
    ...
    Utility:Tween(MainContainer, {Size = UDim2.new(0, 520, 0, 420)}, Config.AnimationSpeed * 1.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    ...
    local PlayerInfoContainer = Utility:Create("Frame", {
        ...
        Position = UDim2.new(1, -155, 1, -48),
        Size = UDim2.new(0, 143, 0, 36),
        ZIndex = 10
    }, {
        Utility:Create("UICorner", { CornerRadius = UDim.new(0, Config.CornerRadius) }),
        Utility:Create("UIStroke", { Color = theme.Border, Transparency = theme.BorderTransparency + 0.2, Thickness = 1 }),
        Utility:Create("UIPadding", { PaddingLeft = UDim.new(0, 38), PaddingRight = UDim.new(0, 8) })
    })
    ...
    CreateControlButton("Minimize", "-", theme.Warning, function()
        ...
        if Window.Minimized then
            Utility:Tween(MainContainer, {Size = UDim2.new(0, 520, 0, 72)}, Config.AnimationSpeed)
            PlayerInfoContainer.Visible = false
        else
            Utility:Tween(MainContainer, {Size = UDim2.new(0, 520, 0, 420)}, Config.AnimationSpeed)
    ...
    function Window:Notify(notifyOptions)
        ...
        local ContentContainer = Utility:Create("Frame", {
            ...
            Utility:Create("UIPadding", { PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 12), PaddingRight = UDim.new(0, 8) }),
    ...
    function Window:CreateTab(tabOptions)
        ...
        local function SelectTab()
            ...
            if oldTab then
                ...
                local slideOutY = goingDown and -50 or 50
                Utility:Tween(oldTab.Content, {Position = UDim2.new(0, 12, 0, slideOutY)}, Config.AnimationSpeed * 0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
                
                task.delay(Config.AnimationSpeed * 0.5, function()
                    if oldTab.Content then
                        oldTab.Content.Visible = false
                        oldTab.Content.Position = UDim2.new(0, 12, 0, 12)
                    end
                end)
            end
            ...
            local slideInY = goingDown and 50 or -50
            TabContent.Position = UDim2.new(0, 12, 0, slideInY)
            TabContent.Visible = true
            TabContent.CanvasPosition = Vector2.new(0, 0)
            
            Utility:Tween(TabContent, {Position = UDim2.new(0, 12, 0, 12)}, Config.AnimationSpeed * 0.8, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
        end
        ...
        function Tab:CreateSection(sectionOptions)
            ...
            function Section:CreateButton(opts)
                ...
                ClickArea.MouseEnter:Connect(function()
                    ...
                    if Icon:IsA("TextLabel") then
                        Utility:Tween(Icon, {Position = UDim2.new(1, -30, 0, 0)}, Config.AnimationSpeed * 0.5)
                    end
                end)
            ...
            function Section:CreateToggle(opts)
                ...
                local function Update(anim)
                    ...
                    if Toggle.Value then
                        ...
                        Utility:Tween(Circle, {Position = UDim2.new(1, -19, 0.5, 0)}, dur, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
                        ...
                        Utility:Tween(Circle, {Position = UDim2.new(0, 3, 0.5, 0)}, dur, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
            ...
            function Section:CreateSlider(opts)
                ...
                local function UpdateSlider(inputPos)
                    local pos = math.clamp((inputPos.X - BarBg.AbsolutePosition.X) / BarBg.AbsoluteSize.X, 0, 1)
                ...
                Input.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        dragging = true
                        Utility:Tween(Knob, {Size = UDim2.new(0, 20, 0, 20)}, Config.AnimationSpeed * 0.3)
                ...
                local inputEndedConn = UserInputService.InputEnded:Connect(function(input)
                    if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and dragging then
                        dragging = false
                        Utility:Tween(Knob, {Size = UDim2.new(0, 16, 0, 16)}, Config.AnimationSpeed * 0.3)
                    end
                end)
    ...
    function Window:Toggle(visible)
        ...
        if visible then
            ...
            Utility:Tween(MainContainer, {Size = UDim2.new(0, 520, 0, 420)}, Config.AnimationSpeed, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        else
            Utility:Tween(MainContainer, {Size = UDim2.new(0, 0, 0, 0)}, Config.AnimationSpeed, Enum.EasingStyle.Back, Enum.EasingDirection.In)
            task.wait(Config.AnimationSpeed + 0.1)
...
function PickleLibrary:GetThemes()
    local list = {}
    for n, _ in pairs(Themes) do table.insert(list, n) end
    return list
end

function PickleLibrary:AddTheme(name, data)
    Themes[name] = data
end

function PickleLibrary:IsMobile()
